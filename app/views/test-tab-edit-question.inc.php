<?php 
    $quest_type = 'Edit subjective Question'; 
    if(isset($_GET['type']) && $_GET['type'] == 'objective') {
        $quest_type = 'Edit objective Question';
    } elseif(isset($_GET['type']) && $_GET['type'] == 'multiple') {
        $quest_type = 'Edit multiple Choice';
    }
?>
...
<?php if(is_object($question)): ?>
    ...
    <form action="" class="form w-100" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
            <textarea class="from-control w-100" name="question" placeholder="Type your question here.." id="" cols="30" rows="5"><?= get_var('question', $question->question) ?></textarea>
        </div>
        ...
        <div class="input-group mb-3">
            ...
            <input type="text" name="comment" placeholder="Leave a comment" value="<?= get_var('comment', $question->comment) ?>" class="form-control" id="">
        </div>
        <div class="input-group mb-3">
        ...
        <div class="p-4">
            <img style="width: 30%; height: 30%;" src="<?= URLROOT . '/' . $question->image ?>" class="d-block mx-auto" alt="no-image" >
        </div>
        <?php endif; ?>
        ...
        <div class="card" style="width: 100%;">
            ...
            <ul class="list-group list-group-flush choice-list">
                <?php if(isset($_POST['choice0'])): 
                 ...
                 <?php else: ?>
                    <?php 
                        $choices = json_decode($question->choices);  //show($data); 
                        $letters = ['A', 'B', 'C', 'D', 'E', 'F', 'I', 'J'];
                        //die;
                        $num = 0;
                        ?>
                    <?php foreach($choices as $letters => $value): ?>
                        <li class="list-group-item">
                            <?=$letters?>: 
                            <input type="text" name="choice<?= $num ?>" id="" value="<?= $value ?>" class="w-100 form-control" placeholder="Type your answer here..">
                            <label style="cursor: pointer;"><input type="radio" <?= $question->correct_answer == $letters ? 'checked' : '' ?> value="<?= $letters ?>"  name="correct_answer" id=""> Correct_answer</label>
                        </li>
                    <?php $num++; endforeach; ?>
                <?php endif; ?>
            </ul>
    ...
    <?php endif; ?>
        ...
        <button class="btn btn-sm btn-danger float-end" type="submit">Edit Question</button>
    </form>
    <?php else: ?>
        <center>
            <div class="mb-3">
